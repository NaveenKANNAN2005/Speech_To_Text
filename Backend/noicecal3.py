...
def create_fastapi_app() -> "FastAPI":
    ...
    async def live_captioning(websocket: WebSocket):
        ...
        try:
            ...
            logging.info(f"Received live-captioning options: {data}")
            options = json.loads(data)
        except Exception as e:
            logging.error(f"Error receiving live caption options: {str(e)}")
        ...
        def callback(chunk: str):
            loop.call_soon_threadsafe(asyncio.create_task, send_chunk(chunk))
        try:
            recognizer.live_caption_with_callback(options, callback)
            await websocket.close()
        except Exception as e:
            logging.error(f"Live captioning error: {str(e)}")
            await websocket.send_json({"text": "Error during live captioning."})
            await websocket.close()
